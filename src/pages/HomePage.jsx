...
const HomePage = () => {
  ...
  const favorite = blogs.filter((blog) => blog.liked);
    ...
    <section>
      {blogs.length === 0 ? (
        ...
        <>
          ...
          <div className='pt-10'>
            <div className='border-2 border-[#0259aa] w-32 h-9 text-center rounded-lg text-xl text-[#0259aa]'>Favorite</div>
            {favorite.length === 0 ? (
              <div className='text-center text-[#0259aa] text-3xl pt-10'>No favorites yet.</div>
            ) : (
              favorite.map((blog) => (
                <div key={blog.id}>
                  <div className='flex flex-col gap-5'>
